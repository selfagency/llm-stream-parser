...
| | **Active total (Phases 3–23)** | | **~151.5 SP** | 11–12 sprints | | | 43 of 43 findings + ethical commitments + ingress scanning + langeval eval + ACP depth | |
| | **Deferred total (Phases 24–28)** | | **~93 SP** | ~8 sprints (post-v1) | | | server mode + network proxy + A2A + self-improvement + supply-chain | DEFERRED |

**Sprint capacity assumption**: 3-person team, 2-week sprints, ~20 SP/person/sprint after meetings/review/oncall = ~60 SP/sprint. Active scope (~151.5 SP) ≈ 2.5 sprints of pure capacity, spread across 11–12 calendar sprints because phases have dependencies. Buffer is 0 SP after the 15-competitor extensions and langeval integration — see §30.4 burndown for descope guidance (recommend extending to Sprint 12 or descoping P3 items).
...
|---|---|---|---|
| 1 | ~15 (P3+P5+P7 start + P19 start) | 15 | 136.5 |
| 2 | ~18 (P4+P5/P7/P19 finish + P20 start) | 33 | 118.5 |
| 3 | ~14 (P6+P8+P20 finish+P22) | 47 | 104.5 |
| 4 | ~13 (P9 start+P21 start) | 60 | 91.5 |
| 5 | ~17 (P9 finish+P10 with §15.7 ingress extension+P21 finish) | 77 | 74.5 |
| 6 | ~10 (P11+P12) | 87 | 64.5 |
| 7 | ~18.5 (P13 with §18.7 langeval integration + P14 start with §19.10 ACP ledger + P23 start) | 105.5 | 46 |
| 8 | ~14 (P14 finish with ACP translators + P15 start + P23 finish) | 119.5 | 32 |
| 9 | ~12 (P15 finish+P16+P17 start) | 131.5 | 20 |
| 10 | ~12 (P17 finish+P18 start) | 143.5 | 8 |
| 11 | ~8 (P18 finish) | 151.5 | 0 |

Active total: ~151.5 SP (Phases 3–23, including Phase 10 §15.7 ingress extension +3 SP, Phase 13 §18.7 langeval integration +3.5 SP, Phase 14 §19.10 ACP event ledger +5 SP). Buffer is now 0 SP — the extensions from the 15-competitor comparison and langeval integration consumed the remaining buffer. **Recommend extending the timeline by 1 sprint (Sprint 12) or descoping P3 items (Phase 18 image/audio, Phase 22) to recover ~9 SP of buffer.** Phase 20 (ethical policy), Phase 10 §15.7 (ingress scanning), Phase 13 §18.7 (langeval eval), and Phase 14 §19.10 (ACP depth) are all P0/P1 and must not be descoped — they close critical competitive gaps surfaced by the expanded 15-competitor comparison. The langeval integration replaces a custom eval build with a more capable external platform — net +0.5 SP but delivers persona simulation, red-teaming, Battle Arena, and GEPA prompt optimization that would never have been built in-house.
...
- [ ] `quarantine` and `allow-with-approval` are distinct states in `GuardrailResult`
- [ ] `transformReason` distinguishes `redaction` | `rewrite` | `normalization`
- [ ] `escalate` is differentiated from `block` in the runtime hook
- [ ] `JsonlAuditLogger` persists receipts with PII/secret redaction
- [ ] One canonical `GuardrailsConfig`; duplicate in `packages/shared` removed
- [ ] README matches actual exports; Policy Enforcement Status table present
...
2. **Gateway** — 7 routing strategies, replica scoring, health tracking, circuit breaker, quota enforcement. Phase 5 moves this into the daemon. hermes-agent has a gateway but without the routing strategy breadth.
...
## 40. (Superseded by §44 below — Phases 24–28 implementation order)
